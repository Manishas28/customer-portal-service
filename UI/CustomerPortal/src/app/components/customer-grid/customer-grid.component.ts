...
@Component({
  ...
  standalone: true,
  imports: [HttpClientModule, CommonModule,ReactiveFormsModule, NgxPaginationModule],
...
export class CustomerGridComponent {
  ...
  constructor(private customerService: CustomerService, private fb: FormBuilder,
     private route: ActivatedRoute, private router: Router) {
      ...
      this.customerService.getTotalCustomersCount().subscribe(total => this.pageConfig.totalItems = total);
   }
    ...
    this.route.queryParams.subscribe(params =>
      {
        ...
        this.retrieveCustomers(this.pageConfig);
      }
    );
  }
  ...
  retrieveCustomers(config: any): void {
    this.customerService.getCustomers(config.currentPage, config.itemsPerPage).subscribe(customers => this.customers = customers);
  }
  ...
  update() {
    if(!this.isEditing) {
      ...
      let index = this.customers.map(u => u.id).indexOf(this.customerSelected.id);
      ...
      if(this.customerSelected.id){
        this.customerService.updateCustomer(this.customerSelected.id, this.customers[index]).subscribe(() => this.retrieveCustomers(this.pageConfig));
      }
    }
  ...
  deleteCustomer(index: number) {
    if(confirm('Are you sure you want to delete this customer?')) {
      ...
      if(customerId){
        this.customerService.deleteCustomer(customerId).subscribe(() => this.retrieveCustomers(this.pageConfig));
      }
